...
function footer(site, variant) {
  ...
  <div class="${wrapCls} footer-inner">
    ...
    <div class="footer-right">
      <a class="footer-rss" href="/rss.xml" title="Nguồn cập nhật (RSS)">RSS</a>
...
/* ───────────────────────── Trang chủ (Home) ─────────────────────────
   Hiển thị TOÀN BỘ nội dung (cả Cập nhật lẫn Hướng dẫn). Phần "Ghim" chỉ
   xuất hiện ở đây và lấy theo các bài được ghim mới nhất. */
export function renderIndex(site, posts) {
  // Bài ghim, mới nhất trước.
  const pinned = posts
    .filter((p) => p.pinned)
    .sort((a, b) => String(b.date).localeCompare(String(a.date)));
  // Banner lớn: bài ghim mới nhất; nếu không có bài ghim thì lấy bài mới nhất.
  const featuredPost = pinned.length ? pinned[0] : posts[0] || null;
  const ghimRest = pinned.slice(1); // các bài ghim còn lại (ngoài banner)

  // Các bài đã được "đưa lên trên" (banner + ghim) sẽ ẩn khỏi lưới khi chưa lọc.
  const promoted = featuredPost
    ? pinned.length
      ? pinned.map((p) => p.slug)
      : [featuredPost.slug]
    : [];
  ...
  const featuredBlock = featuredPost
    ? featuredCard(featuredPost, pinned.length ? '📌 Ghim · Nội dung quan trọng' : undefined)
    : '';
...
${
  ghimRest.length
    ? `<div class="ghim-more-label">${SVG.pin}<span>Các nội dung đang ghim khác</span></div>
<section class="grid grid--ghim" aria-label="Các bài đang ghim">
${ghimRest.map(postCard).join('\n')}
</section>`
    : ''
}
</div>`
