...
function postCard(post) {
  ...
  const vars = `--mc:${mc};--mc-soft:${hexA(mc, 0.1)};--mc-line:${hexA(mc, 0.14)};--mc-ghost:${hexA(mc, 0.22)};`;
  ...
  // Bài có ảnh bìa -> dùng ảnh làm banner; bài chưa có -> nền minh hoạ gradient + icon.
  // Riêng bài video vẫn hiện nút play chồng lên ảnh.
  const showIcon = !hasCover || post.mediaType === 'video';
  ...
  <div class="card-media${hasCover ? ' has-cover' : ''}">
    ${hasCover ? `<img class="card-cover" src="${escapeHtml(post.cover)}" alt="" loading="lazy">` : ''}
    ${post.isNew ? '<span class="card-new">● MỚI</span>' : ''}
    <span class="card-type">${t.short}</span>
    ${showIcon ? `<span class="card-icon">${icon}</span>` : ''}
  </div>
  <div class="card-body">
    <div class="card-tags">
...
function featuredCard(post, tagOverride) {
  ...
  const hasCover = !!post.cover;
  // Có ảnh bìa -> phủ kín khung media của banner; video vẫn giữ nút play chồng lên ảnh.
  const showMediaInner = !hasCover || post.mediaType === 'video';
  ...
  <div class="featured-inner">
    ...
    <div class="featured-media${hasCover ? ' has-cover' : ''}">
      ${hasCover ? `<img class="featured-cover" src="${escapeHtml(post.cover)}" alt="" loading="lazy">` : ''}
      ${showMediaInner ? mediaInner : ''}
...
export function renderPost(site, post, nav = {}) {
  ...
  const sideInfo = `<div class="side-card side-card--ver">
    <div class="side-title-row">
      <span class="side-title">Thông tin phiên bản</span>
      <span class="side-ver">${escapeHtml(post.version || '—')}</span>
    </div>
    <div class="info-list">
      <div class="info-row"><span class="k">Danh mục</span><span class="v" style="color:${mc};">${escapeHtml(post.module || '—')}</span></div>
      ...
      <div class="info-row"><span class="k">Yêu cầu từ</span><span class="v">${escapeHtml(post.department || '—')}</span></div>
    </div>
  </div>`;
    ...
    <div class="author">
      <div class="author-id">
        ...
        <span>
          ...
          <span class="author-meta">Phát hành ${formatDateVi(post.date)}</span>
        </span>
      </div>
    </div>
  </header>
