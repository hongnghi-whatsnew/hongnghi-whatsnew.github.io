...
async function listPosts() {
  ...
  posts.sort(
    (a, b) => (b.pinned ? 1 : 0) - (a.pinned ? 1 : 0) || String(b.date).localeCompare(String(a.date)),
  );
  return posts;
}
...
export async function handleAdmin(req, res) {
  ...
  try {
    ...
    if (p === '/api/posts' && req.method === 'POST') {
      const slug = await createPost(await readBody(req));
      sendJson(res, 200, { ok: true, slug, build: await rebuild() });
      return true;
    }
    if (p === '/api/post' && req.method === 'PUT') {
      const slug = await updatePost(url.searchParams.get('file') || '', await readBody(req));
      sendJson(res, 200, { ok: true, slug, build: await rebuild() });
      return true;
    }
