...
// Lưu ẢNH BÌA (banner) của bài — gửi dưới dạng data URL base64 — vào public/posts/<slug>/.
// Chỉ nhận tệp ảnh; tên cố định "cover.<ext>" để lần tải sau ghi đè, không sinh file thừa.
async function saveCoverImage(slug, upload) {
  const m = /^data:(image\/[a-z0-9.+-]+);base64,(.*)$/is.exec(String((upload && upload.dataUrl) || ''));
  if (!m) throw new Error('Ảnh bìa không hợp lệ (chỉ nhận tệp ảnh).');
  const ext = IMAGE_EXT[m[1].toLowerCase()];
  if (!ext) throw new Error('Định dạng ảnh bìa không hỗ trợ. Hãy dùng PNG, JPG, WEBP, GIF hoặc SVG.');
  const buf = Buffer.from(m[2], 'base64');
  if (!buf.length) throw new Error('Ảnh bìa bị rỗng.');
  if (buf.length > MAX_IMAGE) throw new Error('Ảnh bìa quá lớn (vượt 15 MB). Hãy giảm dung lượng rồi tải lại.');
  const dir = path.join(PUBLIC_POSTS_DIR, slug);
  await mkdir(dir, { recursive: true });
...
// Trả về đường dẫn ảnh bìa cuối cùng: gỡ / tải mới / giữ nguyên đường dẫn cũ.
async function resolveCover(slug, p) {
  if (p.coverRemove) return '';
  if (p.coverUpload && p.coverUpload.dataUrl) return saveCoverImage(slug, p.coverUpload);
  return String(p.cover || '');
}
...
async function createPost(p) {
  ...
  const cover = await resolveCover(slug, p);
  await writePost(slug, { ...p, ...files, cover });
